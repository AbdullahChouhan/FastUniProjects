...
void DoublyLinkedList::deleteParticular(Node* nptr) {
    ...
    nptr->data = next->data;
    nptr->next = next->next;
    nptr->next->prev = nptr; // Because this is doubly linked list
    delete next;
    size--;
}


void DoublyLinkedList::deleteNode(char val) {
    Node *ptr = head;
    while (ptr != nullptr) {
        if (ptr->data == val) {
...
char* DoublyLinkedList::getNextLargestNodes() {
    if (head == nullptr) return nullptr;
    char* nextLargest = new char[size];
